	       ...
	       int sibling_count_hint)
{
	...
	if (p == current) {
		...
		/*
		 * Skip walt_try_to_wake_up() here: p == current == rq->curr in
		 * this fast path, so there is no scheduling transition for WALT
		 * to account, and calling it would reacquire rq->lock, negating
		 * the lock-free wakeup optimization from 997c85f9317d.
		 */
		p->state = TASK_RUNNING;
		trace_sched_wakeup(p);
		goto out;
